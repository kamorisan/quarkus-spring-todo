...
echo "========================================="
echo "  Deploying Quarkus JVM to OpenShift"
if [ "$DEPLOY_MODE" = "serverless" ]; then
...
spec:
  source:
    ...
    git:
      uri: ${GIT_REPO}
      ref: ${GIT_BRANCH}
    contextDir: openshift/quarkus-jvm
  strategy:
    type: Docker
    dockerStrategy:
...
if [ "$DEPLOY_MODE" = "serverless" ]; then
    ...
    if [ -n "$SERVICE_URL" ]; then
        ...
        echo "    -d '{\"title\":\"Test from Serverless\",\"description\":\"Quarkus JVM on OpenShift\"}'"
        echo ""
    else
    ...
    if [ -n "$ROUTE_URL" ]; then
        ...
        echo "    -H 'Content-Type: application/json' \\"
        echo "    -d '{\"title\":\"Test from OpenShift\",\"description\":\"Quarkus JVM Standard Deployment\"}'"
        echo ""
    else
