...
# ========================================
# Spring Boot JVM Deployment Script
...
echo "========================================="
echo "  Deploying Spring Boot JVM to OpenShift"
if [ "$DEPLOY_MODE" = "serverless" ]; then
...
spec:
  source:
    ...
    git:
      uri: ${GIT_REPO}
      ref: ${GIT_BRANCH}
    contextDir: openshift/spring-jvm
  strategy:
    type: Docker
    dockerStrategy:
...
if [ "$DEPLOY_MODE" = "serverless" ]; then
    ...
    if [ -n "$SERVICE_URL" ]; then
        ...
        echo "  curl $SERVICE_URL/actuator/health/readiness"
        echo "  curl $SERVICE_URL/api/todos"
        echo ""
        echo "Create a todo:"
        echo "  curl -X POST $SERVICE_URL/api/todos \\"
        echo "    -H 'Content-Type: application/json' \\"
        echo "    -d '{\"title\":\"Test from Serverless\",\"description\":\"Spring Boot JVM on OpenShift\"}'"
        echo ""
    else
    ...
    if [ -n "$ROUTE_URL" ]; then
        ...
        echo ""
        echo "Test the service:"
        echo "  curl https://$ROUTE_URL/actuator/health/readiness"
        echo "  curl https://$ROUTE_URL/api/todos"
        echo ""
        echo "Create a todo:"
        echo "  curl -X POST https://$ROUTE_URL/api/todos \\"
        echo "    -H 'Content-Type: application/json' \\"
        echo "    -d '{\"title\":\"Test from OpenShift\",\"description\":\"Spring Boot JVM Standard Deployment\"}'"
        echo ""
    else
